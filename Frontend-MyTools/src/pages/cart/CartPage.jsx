...
const CartPage = () => {
  ...
  const total = useMemo(() => items.reduce((sum, item) => {
    const days = item.listingType === 'RENT' && item.startDate && item.endDate
  ...
  if (loading) return <div className="container-custom py-16 text-center">Loading cart...</div>;
    ...
    <div className="min-h-screen bg-[#fafaf9] dark:bg-[#1a1816] py-12">
      <div className="container-custom max-w-5xl">
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="p-3 rounded-2xl bg-gradient-to-br from-[#6d2842] to-[#a64d6d] text-white">
            <ShoppingCart />
          </div>
          <div>
            <h1 className="text-3xl font-bold text-[#2d2a27] dark:text-white">Shopping Cart</h1>
        ...
        {items.length === 0 ? (
          <div className="bg-white dark:bg-[#2d2a27] rounded-3xl p-12 text-center border border-[#e8e7e5] dark:border-[#4a4642]">
            ...
            <Link to="/Products" className="inline-flex px-5 py-3 rounded-xl bg-[#6d2842] text-white font-semibold">Browse products</Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <motion.div key={`${item.productId}-${item.startDate || ''}`} layout className="bg-white dark:bg-[#2d2a27] rounded-2xl p-5 border border-[#e8e7e5] dark:border-[#4a4642] flex items-center justify-between gap-4">
                  <div>
                    ...
                    {item.listingType === 'RENT' && (
                      <p className="text-xs text-[#8a8580]">{item.startDate} → {item.endDate}</p>
                    )}
                  </div>
            ...
            <div className="bg-white dark:bg-[#2d2a27] rounded-3xl p-6 border border-[#e8e7e5] dark:border-[#4a4642] h-fit space-y-4">
              <h2 className="text-xl font-bold text-[#2d2a27] dark:text-white">Summary</h2>
              <div className="flex justify-between text-lg"><span>Total</span><strong>{total.toFixed(2)} MAD</strong></div>
              <textarea value={shippingAddress} onChange={(e) => setShippingAddress(e.target.value)} rows={3} placeholder="Shipping address / notes" className="w-full rounded-xl border p-3 bg-transparent" />
