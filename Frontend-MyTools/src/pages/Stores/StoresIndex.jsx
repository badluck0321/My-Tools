    ...
    <div className="min-h-screen bg-[#fafaf9] dark:bg-[#1a1816] py-12">
      <div className="container-custom">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-4xl font-bold text-[#2d2a27] dark:text-white">Stores</h1>
            <p className="text-[#8a8580]">Browse verified and active tool stores</p>
          </div>
        </div>
              ...
              return (
                <Link key={store.id} to={target} className="rounded-3xl bg-white dark:bg-[#2d2a27] p-6 border border-[#e8e7e5] dark:border-[#4a4642] hover:shadow-xl transition-shadow">
                  <div className="flex items-start justify-between gap-3 mb-4">
                    ...
                    {store.isVerified && <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-green-50 text-green-700 text-xs font-bold"><ShieldCheck size={14} /> Verified</span>}
                  </div>
                  <h2 className="text-xl font-bold text-[#2d2a27] dark:text-white">{store.name || 'Unnamed store'}</h2>
                  <div className="mt-4 space-y-2 text-sm text-[#5d5955] dark:text-[#c4bfb9]">
                    <p className="flex items-center gap-2"><Mail size={15} /> {store.email || 'No email'}</p>
                    <p className="flex items-center gap-2"><MapPin size={15} /> {store.address || 'No address'}</p>
                  </div>
                  <div className="mt-4 text-xs text-[#8a8580]">
                    {store.isActive === false ? 'Inactive' : 'Active'} · {store.isDeliveryAvailable ? 'Delivery available' : 'No delivery info'}
                  </div>
                </Link>
              );
            })}
