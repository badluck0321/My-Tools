...
const DASHBOARD_TABS = [
  { id: 'profile', name: 'My Profile', icon: User, color: 'from-[#6d2842] to-[#a64d6d]' },
  { id: 'MyStore', name: 'My Store', icon: Store, color: 'from-[#b8862f] to-[#d4a343]', sellerOnly: true },
  { id: 'MyProducts', name: 'My Products', icon: Package, color: 'from-[#b8862f] to-[#d4a343]', sellerOnly: true },
  { id: 'MyMasteries', name: 'My Masteries', icon: Sparkles, color: 'from-[#508978] to-[#70a596]', sellerOnly: true },
  { id: 'favorites', name: 'Favorites', icon: Heart, color: 'from-[#508978] to-[#70a596]' },
  { id: 'orders', name: 'Orders', icon: ShoppingBag, color: 'from-[#6d2842] to-[#b8862f]' },
  { id: 'bookings', name: 'Bookings', icon: CalendarDays, color: 'from-[#508978] to-[#70a596]' },
  { id: 'analytics', name: 'Analytics', icon: BarChart3, color: 'from-[#6d2842] to-[#a64d6d]', sellerOnly: true },
  { id: 'vendor-verification', name: 'Vendor Verification', icon: BadgeCheck, color: 'from-[#6d2842] to-[#a64d6d]' },
  { id: 'admin', name: 'Admin Panel', icon: Shield, color: 'from-[#4a4642] to-[#6d6762]', adminOnly: true },
  { id: 'lookups', name: 'Lookups', icon: ListTree, color: 'from-[#4a4642] to-[#6d6762]', adminOnly: true },
  { id: 'notifications', name: 'Notifications', icon: Bell, color: 'from-[#b8862f] to-[#d4a343]' },
  { id: 'messages', name: 'Messages', icon: MessageCircle, color: 'from-[#508978] to-[#70a596]' },
  { id: 'settings', name: 'Settings', icon: Settings, color: 'from-[#4a4642] to-[#6d6762]' },
];

const Dashboard = () => {
  const { user, isAdmin, isStoreOwner } = useKeycloak();
            ...
            >
              <nav className="space-y-2">
                {visibleTabs.map((tab, index) => (
                  <motion.div key={tab.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.04 }}>
                    <Link
                      to={`/dashboard/${tab.id}`}
                    ...
                    >
                      ...
                      {activeTab === tab.id && <motion.div layoutId="activeTab" className="ml-auto w-1.5 h-1.5 rounded-full bg-white shadow-lg shadow-white/50" />}
                    </Link>
                  </motion.div>
                ))}
              </nav>
